...
//    private static final int EDGE_SIZE = 20; // dp
    ...
    public abstract static class Callback {
         ...
         *
         * @param changedView Component whose position changed
         ...
         */
        public void onViewCaptured(Component capturedChild, int activePointerId) {
        }
         ...
         * <p>
         * <p>If this method returns true, a call to {@link #onViewCaptured(Component, int)}
     ...
     */
    public static ViewDragHelper create(ComponentContainer forParent, Callback cb) {
     ...
     * @return a new ViewDragHelper instance
     */
    public static ViewDragHelper create(ZBannerRaw forParent, float sensitivity, Callback cb) {
     ...
     * @param forParent Parent view to monitor
     */
    private ViewDragHelper(Context context, ComponentContainer forParent, Callback cb) {
        if (forParent == null) {
     ...
     * but {@link Callback#tryCaptureView(Component, int)} will not be asked permission to
     * capture this view.
     *
     * @param childView       Child view to capture
     * @param activePointerId ID of the pointer that is dragging the captured child view
     */
    public void captureChildView(Component childView, int activePointerId) {
        if (childView.getComponentParent() != mParentView) {
     ...
     */
    public Component getCapturedView() {
     ...
     * {processTouchEvent(MotionEvent)} receiving an ACTION_CANCEL event.
     */
    public void cancel() {
    ...
    public void abort() {
        ...
        if (mDragState == STATE_SETTLING) {
            final int oldX = mScroller.getCurrValue(ScrollHelper.AXIS_X);
            final int oldY = mScroller.getCurrValue(ScrollHelper.AXIS_Y);
     ...
     */
    public boolean smoothSlideViewTo(Component child, int finalLeft, int finalTop) {
        mCapturedView = child;
        mActivePointerId = INVALID_POINTER;
    ...
    public boolean settleCapturedViewAt(int finalLeft, int finalTop) {
        ...
        return forceSettleCapturedViewAt(finalLeft, finalTop,
                (int) mVelocityTracker.getHorizontalVelocity(),
                (int) mVelocityTracker.getVerticalVelocity());
    }
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        ...
        mScroller.doFling(mCapturedView.getLeft(), mCapturedView.getTop(),
                (int) mVelocityTracker.getHorizontalVelocity(),
                (int) mVelocityTracker.getVerticalVelocity(),
     ...
     * { Component#computeScroll()} or similar methods
     * invoked as part of layout or drawing.
     * @return true if settle is still in progress
     */
    EventHandler mEventHandler = new EventHandler(EventRunner.getMainEventRunner());

    public boolean continueSettling(boolean deferCallbacks) {
        if (mDragState == STATE_SETTLING) {

            boolean keepGoing = mScroller.isFinished();

            final int x = mScroller.getCurrValue(ScrollHelper.AXIS_X);
            final int y = mScroller.getCurrValue(ScrollHelper.AXIS_Y);
            ...
            if (keepGoing && x == mScroller.getScrollDistanceX() && y == mScroller.getScrollDistanceY()) {
            ...
            cognifunc(keepGoing, deferCallbacks);
        }

        return mDragState == STATE_SETTLING;
    }

    // this func is used to counter congitive complexity .
    ...
    private void saveLastMotion(TouchEvent ev) {
     ...
     * {#processTouchEvent(MotionEvent)}. If one of these methods has not
     ...
     *
     * @param toCapture Component to capture
     ...
     *
     * @param v      Component to test for horizontal scrollability
    ...
    protected boolean canScroll(Component v, boolean checkV, int dx, int dy, int x, int y) {
        if (v instanceof ComponentContainer) {
            ...
            final int scrollX = v.getScrollValue(Component.AXIS_X);
            final int scrollY = v.getScrollValue(Component.AXIS_Y);
        ...
        return checkV && (v.canScroll(-dx) || v.canScroll(-dy));
    }
     ...
     */
    public boolean shouldInterceptTouchEvent(TouchEvent ev) {
        ...
        switch (action) {
            ...
            case TouchEvent.OTHER_POINT_DOWN: {
                cognifunc3(point,actionIndex,ev);
                break;
            }

            case TouchEvent.POINT_MOVE: {
                cognifunc2(ev, action);
                break;
            }
    ...
    public void cognifunc3(MmiPoint point, int actionIndex,TouchEvent ev) {
        final int pointerId = ev.getPointerId(actionIndex);
        final float x = point.getX();
        final float y = point.getY();
    ...
    public void cognifunc2(TouchEvent ev, int action) {
        ...
        switch (action) {
            case TouchEvent.POINT_MOVE: {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    if(cognifunc5(pastSlop,toCapture,dx,dy,pointerId) == 0)
                        break;
    ...
    public  int cognifunc5(boolean pastSlop,Component toCapture,float dx, float dy,int pointerId){
        if (pastSlop) {
    ...
    public int cognifunc4(Component toCapture,float dx, float dy){
    ...
    public void cognidun7(TouchEvent ev,int pointerId)
    {
    ...
    public  void cognifunc6(MmiPoint point,TouchEvent ev){
        if (mDragState == STATE_DRAGGING) {
            // If pointer is invalid then skip the ACTION_MOVE.
            if (!isValidPointerForActionMove(mActivePointerId)) return;
 ...
 */
     ...
     * { #shouldInterceptTouchEvent(MotionEvent)} or
     * { #processTouchEvent(MotionEvent)}. You should only rely on
    ...
    private void releaseViewForPointerUp() {
        ...
        final float xvel = clampMag(
                mVelocityTracker.getHorizontalVelocity(),
                mMinVelocity, mMaxVelocity);
        final float yvel = clampMag(
                mVelocityTracker.getVerticalVelocity(),
                mMinVelocity, mMaxVelocity);
    ...
    public Component findTopChildUnder(int x, int y) {
        ...
        for (int i = childCount - 1; i >= 0; i--) {
            final Component child = mParentView.getComponentAt(mCallback.getOrderedChildIndex(i));
    ...
    private boolean isValidPointerForActionMove(int pointerId) {
        if (!isPointerDown(pointerId)) {
            LogUtil.error(TAG, "Ignoring pointerId=" + pointerId + " because ACTION_DOWN was not received "
